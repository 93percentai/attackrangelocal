...
v_disk_device() {
  # Bare block-device name. We build the TOML array in build-iso.sh, so
  # the user never has to get shell-quoting-inside-TOML right.
  if [[ "$1" =~ ^[a-z0-9]+$ ]]; then return 0; fi
  err "must be a bare device name like 'nvme0n1' or 'sda' (no brackets or quotes)"
  return 1
}
v_int_or_empty() {
  [[ -z "$1" ]] && return 0
...
You can rotate it after first boot." \
  ...
  ask DISK_DEVICE "Target install disk" \
"Bare device name of the disk the Proxmox installer will WIPE and install
to. No brackets, no quotes. ext4 installs take exactly one disk.
  nvme0n1  modern laptops + most M.2 SSDs (DEFAULT)
  sda      older laptops, server SATA drives
  vda      VirtIO (nested QEMU/KVM testing)
Boot a Linux live USB on the target box and run
   lsblk -d -o NAME,SIZE,MODEL,TRAN
to confirm the right device name." \
    "${CURRENT[DISK_DEVICE]:-nvme0n1}" plain v_disk_device

  ask PROXMOX_FQDN "Proxmox host FQDN" \
"Fully-qualified hostname the auto-installer writes to /etc/hostname
on the Proxmox VE host. Becomes the Tailscale device name as well
(short part before the first dot), so this is how you'll reach it:
'ssh root@<short>.<tailnet>'. Just press Enter to take the default." \
    "${CURRENT[PROXMOX_FQDN]:-ludus-attackrangelocal.range.local}" plain v_fqdn
...
multiple radios (e.g. wlp2s0, wlp3s0)." \
    ...
    ask WIFI_HIDDEN_SSID "Hidden SSID (not broadcasting)?" \
"Set true if the AP does not broadcast its SSID. Adds scan_ssid=1 to
wpa_supplicant so the network can still be found." \
      "${CURRENT[WIFI_HIDDEN_SSID]:-false}" plain v_bool
...
becomes the default route; wired stays as failover)." \
    ...
    ANSWERS[WIFI_HIDDEN_SSID]="${CURRENT[WIFI_HIDDEN_SSID]:-false}"
    ANSWERS[WIFI_DISABLE_WIRED_AFTER_BOOT]="${CURRENT[WIFI_DISABLE_WIRED_AFTER_BOOT]:-false}"
  fi
...
write_env() {
  ...
  declare -A DEFAULTS=(
    ...
    [WIFI_ENABLE]=false
    [WIFI_COUNTRY]=US
    [WIFI_DISABLE_WIRED_AFTER_BOOT]=false
  ...
  )
  {
    ...
    echo
    for k in RANGE_MODE RANGE_ID PROXMOX_FQDN DISK_DEVICE \
             ...
             WIFI_HIDDEN_SSID WIFI_DISABLE_WIRED_AFTER_BOOT \
...
do_build() {
  ...
  if grep -q 'proxmox-auto-install-assistant: command not found' "$log"; then
    ...
    if grep -qi 'disk-list' "$log"; then
      note "DISK_DEVICE in .env must be a bare device name:  DISK_DEVICE=nvme0n1"
      note "(no brackets, no quotes — build-iso.sh builds the TOML array itself)"
    fi

...
You can rotate it after first boot." \
  ...
  ask DISK_DEVICE_LIST "Target install disk" \
'Single block device the Proxmox installer wipes and installs to.
PVE 8.4 requires exactly one disk for ext4. Common values:
...
to confirm the right device name.' \
    "${CURRENT[DISK_DEVICE_LIST]:-[\"nvme0n1\"]}" plain v_disk_list
...
write_env() {
  ...
  )
  {
    ...
    echo
    for k in RANGE_MODE RANGE_ID PROXMOX_FQDN DISK_DEVICE_LIST \
